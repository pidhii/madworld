...
const char _named_colors_json[] = R"(
{
  "Shades of Black": {
    "Black": "#000000",
    "Charcoal": "#36454F",
...
mw::color_t
mw::gui::parse_color(std::string_view str)
{
  using namespace nlohmann;

  static thread_local std::unordered_map<std::string, std::string> named_colors;
  if (named_colors.empty())
  {
    for (const json &family : json::parse(_named_colors_json))
    {
      for (const auto &[name, hexstr] : family.items())
        named_colors[name] = hexstr;
    }
  }
  ...
  {
    const auto it = named_colors.find(str.data());
    if (it != named_colors.end())
      return _parse_hex_color(it->second);
  }

  error("impossible color: %s", str.data());
  throw std::runtime_error {"impossible color"};
}
